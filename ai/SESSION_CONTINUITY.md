...
`RESUME ACTION` is the immediate safe recovery work; `PROJECT NEXT` is the next action after reconciliation and verification. Reconstruct `ACTIVE`, `WAITING/PAUSED`, `PROJECT NEXT`, material `AFTER` ordering, `OPEN HANDOFFS`, `BLOCKER/BOUNDARY`, and any interrupt/return point. Resolve temporal grounding when relative dates matter: on cold or successor resume, establish the actual current local date/time before interpreting terms such as today, yesterday, tomorrow, tonight, or next week when they could affect reconciliation. Do not require a time lookup for every trivial resume. Then resolve the authoritative branch/current repository state. The newest `PROGRESS.md` entry is not automatically current priority; an intentionally waiting state should not manufacture work.
